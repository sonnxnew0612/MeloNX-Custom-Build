...
namespace Ryujinx.Cpu.LightningJit.Arm64.Target.Arm64
{
    static class CompilerPass
    {
        ...
        public static CompiledFunction Compile(CpuPreset cpuPreset, IMemoryManager memoryManager, ulong address, AddressTable<ulong> funcTable, IntPtr dispatchStubPtr)
        {
            ...
            for (int blockIndex = 0; blockIndex < multiBlock.Blocks.Count; blockIndex++)
            {
                ...
                for (int index = 0; index < instCount; index++)
                {
                    ...
                    if (CanPassThroughDirectly(instInfo, regAlloc))
                    {
                        writer.WriteInstruction(instInfo.Encoding);
                    }
                    else
                    {
        ...
        private static bool CanPassThroughDirectly(InstInfo instInfo, RegisterAllocator regAlloc)
        {
            if (instInfo.AddressForm != AddressForm.None)
                return false;

            if (IsDcInstruction(instInfo.Encoding))
                return false;

            if (instInfo.Name.IsSystem())
                return false;

            if (instInfo.Name.IsControlFlowOrException())
                return false;

            var (gprReadMask, fpSimdReadMask) = RegisterUtils.PopulateReadMasks(
                instInfo.Name, instInfo.Flags, instInfo.Encoding);
            var (gprWriteMask, fpSimdWriteMask) = RegisterUtils.PopulateWriteMasks(
                instInfo.Name, instInfo.Flags, instInfo.Encoding);

            uint allGprMask = gprReadMask | gprWriteMask;

            if ((allGprMask & RegisterUtils.ReservedRegsMask) != 0)
                return false;

            for (int i = 0; i < 32; i++)
            {
                if ((allGprMask & (1u << i)) != 0)
                {
                    if (regAlloc.RemapReservedGprRegister(i) != i)
                        return false;
                }
            }
        ...
        private static bool IsDcInstruction(uint encoding)
        {
            // mrs x0, tpidrro_el0 / tpidr_el0
            if ((encoding & ~0x1fu) == 0xd53bd060 || (encoding & ~0x1fu) == 0xd53bd040)
                return true;
            
            // msr tpidr_el0, x0
            if ((encoding & ~0x1fu) == 0xd51bd040)
                return true;
            
            // mrs x0, ctr_el0 
            if ((encoding & ~0x1fu) == 0xd53b0020)
                return true;
    
            // mrs x0, cntpct_el0
            if ((encoding & ~0x1fu) == 0xd53be020)
                return true;

            if (SysUtils.IsCacheInstEl0(encoding))
                return true;

            if (SysUtils.IsCacheInstUciTrapped(encoding))
                return true;
        ...
        private static int CalculateStackSizeForCallSpill(uint gprUseMask, uint fpSimdUseMask, uint pStateUseMask)
        {
            ...
            int size = BitOperations.PopCount(gprUseMask & ~AbiConstants.GprCalleeSavedRegsMask) * 8 +
                       BitOperations.PopCount(fpSimdUseMask) * 16 +
                       (pStateUseMask != 0 ? 8 : 0);

            // Ensure 16-byte alignment for Apple Silicon
            return (size + 15) & ~15;
        }
