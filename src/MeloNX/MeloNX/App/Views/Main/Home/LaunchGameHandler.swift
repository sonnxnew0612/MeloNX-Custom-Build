...
class LaunchGameHandler: ObservableObject {
    ...
    var shouldCheckJIT: Binding<Bool> {
        Binding(
            get: { self.currentGame != nil && !self.ryujinx.jitenabled && checkAppEntitlement("com.apple.developer.kernel.increased-memory-limit") },
            set: { newValue in
                print(newValue)
            }
        )
    ...
    func checkForGame() -> Bool {
        self.currentGame != nil && self.ryujinx.jitenabled && (self.nativeSettings.showProfileonGame.value ? self.profileSelected : true)
    }
    ...
    private func configureEnvironmentVariables() {
        if nativeSettings.setting(forKey: "MVK_CONFIG_SYNCHRONOUS_QUEUE_SUBMITS", default: false).value {
